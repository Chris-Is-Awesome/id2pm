...
public class RollAction : EntityAction
{
	...
	[SerializeField]
	public float _speed = 5f; // Made public
	...
	[SerializeField]
	LayerMask _rollTraceLayer = default(LayerMask);
	...
	bool CheckSlope(Vector3 dir, out Vector3 newDir)
	{
		...
		{
			Vector3 vector = -bc_TraceHit.normal;
			float num = Vector3.Dot(dir, vector);
			float num2 = Vector3.Dot(Vector3.up, vector);
			if (num > 0.1f && num2 < 1f - this._keepRollSlope)
			{
				this.keepRolling = true;
				vector.y = dir.y;
				newDir = vector.normalized;
				return true;
			}
		}
		newDir = dir;
	...
	protected override bool DoUpdate()
	{
		...
		AnimationCurve animationCurve = (!this.keepRolling) ? this._speedMulCurve : this._loopCurve;
		float num2 = animationCurve.Evaluate(num) * this._speed;

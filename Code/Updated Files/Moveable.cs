...
public class Moveable : EntityComponent, IUpdatable, IBaseUpdateable
{
	...
	[SerializeField]
	public float _moveSpeed = 4f; // Made public

	[SerializeField]
	float _turnSpeed = 1080f;

	[SerializeField]
	bool _turnToDir = true;

	[SerializeField]
	bool _moveInTurnDir;

	[SerializeField]
	float _turnFactor = 1f;

	[SerializeField]
	Vector3 _turnVector = Vector3.forward;
	...
	[SerializeField]
	bool _useDefaultStopAnim = true;

	[SerializeField]
	float _animSpeedScale = 1f;

	[SerializeField]
	bool _wallSlideNormalize;

	[SerializeField]
	MoveEffectObject _moveEffectObject;
	...
	void IUpdatable.UpdateObject()
	{
		...
		if (this.animState != null && Mathf.Abs(this._animSpeedScale) > 1E-05f)
		{
			...
			float num = (this.lastPos - vector).magnitude / Time.deltaTime;
